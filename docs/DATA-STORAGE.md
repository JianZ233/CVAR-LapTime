...
The Orbits Processing screen can show post-processing fields that are not part of the RMonitor scoreboard stream, including result status, uploaded state, points, and some corrections. Race result sheets include a Points column and preserve numeric `points` values supplied with each car in the stored source snapshot, including zero. When the RMonitor snapshot has no points, the result sheet calculates CVAR per-race points: one for starting, one for finishing, and, for the designated Race 3 feature, two points for first through fourth in class or one point for fifth and sixth. Saved steward DNF, DNS, and DQ statuses are respected. Weekend entry and authenticity points are championship totals and are not included on an individual race result sheet.
