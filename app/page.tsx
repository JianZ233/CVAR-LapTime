...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main>
        {activeView === 'timing' ? <TimingBoard snapshot={snapshot} feedState={feedState} secondsAgo={secondsAgo} /> : <ScheduleView />}
      </main>
    </div>
  );
}
...
function TimingBoard({ snapshot, feedState, secondsAgo }: { snapshot: TimingSnapshot; feedState: FeedState; secondsAgo: number }) {
      ...
      <section className="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-[minmax(0,1.5fr)_repeat(3,minmax(150px,0.6fr))]">
        <div className="relative overflow-hidden border border-white/10 bg-card p-5 sm:col-span-2 lg:col-span-1">
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            <span className="font-mono text-sm text-muted-foreground">{snapshot.timeToGo ? `${shortTime(snapshot.timeToGo)} remaining` : snapshot.lapsToGo !== null ? `${snapshot.lapsToGo} laps to go` : 'Session active'}</span>
          </div>
        </div>
  ...
  useEffect(() => {
    ...
    async function refresh() {
      try {
        ...
        setFeedState(Date.now() - new Date(body.snapshot.updatedAt).getTime() > 15_000 ? 'stale' : 'live');
      } catch {
